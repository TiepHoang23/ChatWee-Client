...
const MessageDisplayZone = () => {
  ...
  }, [receiveLength, idRoom]);

  return (
    <div
      className='bg-custom-black-3 w-full h-full flex flex-col-reverse overflow-y-scroll p-4'
      id='scroll'
    >
      <div className='bg-transparent w-full' id='scroll'>
        ...
        {MessageReceive.map((item, index) => {
          ...
          return <MessageItem from={From} key={index} content={item.msg} />;
        })}
      </div>
    </div>
  );
